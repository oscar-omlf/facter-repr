...
def main() -> None:
    ...
    mcfg = MLflowConfig(
        ...
        run_name=f"ml1m_{args.model_id}_seed{args.seed}_{args.predict_mode}",
    )
    ...
    with start_run(
        ...
        for protected_cols in protected_sets:
            ...
            with stage(f"save_outputs[{pset}]", timings):
                out_path = Path(args.processed_dir) / "runs" / f"run_{args.model_id.replace('/', '_')}_{pset}_{args.predict_mode}.parquet"
